...
public class SelectItemsActivity extends AppCompatActivity implements OnItemClickListener, OnItemLongClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.select_all).setOnClickListener(v -> {
			boolean all = VS.ola.list.size() > VS.ola.selected;
			for (HierarchyItemModel him : VS.ola.list) him.setSelected(all);
			VS.ola.selected = all ? VS.ola.list.size() : 0;
			checkSelectVisibility();
		...
		(select = findViewById(R.id.objects_select)).setOnClickListener(v -> {
			...
			boolean ha = VS.ola instanceof HierarchyAdapter;
			...
			for (HierarchyItemModel him : VS.ola.list) {
				if (him.isSelected())
					...
					} else if (!(him.bd instanceof TwoSided) || him.bd instanceof Picture == TestFragment.picTest)
		...
		es = new ExplorerStuff(true, this::setContent, this::checkSelectVisibility, this::checkSelectVisibility, VS, backLog,
		...
		if (none) {
			...
			es.lv.setAdapter(VS.ola);
			VS.ola.occ = this::checkSelectVisibility;
	...
	private void checkSelectVisibility() {
		boolean selectOn = VS.ola.selected > 0 && (!backLog.path.isEmpty() || VS.ola.selected < 2);
	...
	public void onBackPressed() {
		...
		if (!backLog.path.isEmpty() && list.isEmpty() || backLog.path.size() > 1) {
			if (VS.ola instanceof SearchAdapter) {
				setContent(backLog.path.get(-1), (Container) backLog.path.get(-2), backLog.path.size());
				return;
			}
	...
	public void onItemClick(AdapterView<?> par, View v, int pos, long id) {
		HierarchyItemModel him = VS.ola.list.get(pos);
		BasicData bd = him.bd;
		...
		if (bd instanceof Word) {
			...
			} else if (!(ref = VS.ola instanceof SearchAdapter)) backLog.add(false, bd, null);
			else {
	...
	public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
		if (VS.ola instanceof SearchAdapter) {
			...
			es.onChange(true);
		} else if (VS.ola instanceof HierarchyAdapter) {
	...
	public void setContent(BasicData bd, Container parent, int size) {
		es.svc.update(size == 0);
		es.lv.setAdapter(VS.aa = VS.ola = new HierarchyAdapter(es.context, size == 0 ?
				convert(new ArrayList<>(MainChapter.ELEMENTS), null) : convert(((Container) bd)
				.getChildren(parent), (Container) bd), this::checkSelectVisibility, true));
		checkSelectVisibility();
	}
}
